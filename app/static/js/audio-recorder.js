...
export async function startAudioRecorderWorklet(audioRecorderHandler) {
  ...
  // Connect the worklet to a silent destination so the processor gets scheduled
  const silentGain = audioRecorderContext.createGain();
  silentGain.gain.value = 0;
  audioRecorderNode.connect(silentGain).connect(audioRecorderContext.destination);
